            ...
            else
            {
                ...
                using (var chainHolder = new X509ChainHolder())
                {
                    ...
                    else if (!isSignatureTimeValid)
                    {
                        ...
                        {
                            issues.Add(SignatureLog.Issue(!settings.AllowUntrusted, NuGetLogCode.NU3011, string.Format(CultureInfo.CurrentCulture, Strings.VerifyError_SignatureNotTimeValid, FriendlyName)));
                        }
